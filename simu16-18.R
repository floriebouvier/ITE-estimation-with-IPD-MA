...
#### scenario 16: 10 covariates & tte outcome & size = 2800 ####
...
res.na.sl16 = foreach(j = 1:m,
                      ...
                      .packages = c("tidyverse","Hmisc","magrittr","gtools","survival","msm","mvmeta")) %dopar% {
                        ...
                        df$log_hr <- b1*df$age+b2*df$sex+b3*df$sbp+b4*df$mi+b5*df$stroke+b6*df$smoke+
                          b7*df$diab+b8*df$lvhn1+b9*df$height+b10*df$treat+
                        ...
                        df$ite <- expit(b1*df$age+b2*df$sex+b3*df$sbp+b4*df$mi+b5*df$stroke+b6*df$smoke+
                                        b7*df$diab+b8*df$lvhn1+b9*df$height+b10+
                                        b11*df$age+b12*df$sex+b13*df$sbp+b14*df$smoke) -
                                  expit(b1*df$age+b2*df$sex+b3*df$sbp+b4*df$mi+b5*df$stroke+b6*df$smoke+
                                        b7*df$diab+b8*df$lvhn1+b9*df$height)
...
stopCluster(cl)
res.na.sl.sim16 <- t(res.na.sl16[1:25, ])
se.na.sl.sim16 <- t(res.na.sl16[26:44, ])

#### random effects model ####
m <- 100
cl = makeCluster(detectCores())
registerDoParallel(cl)
res.re.sl16 = foreach(j = 1:m,
                      .final = function(l) {do.call("cbind", lapply(
                        l[sapply(l,`[[`, "success")],
                        function(subl) c(subl$res, subl$seed)))},
...
stopCluster(cl)
res.re.sl.sim16 <- t(res.re.sl16[1:25, ])
...
stopCluster(cl)
res.si.sl.sim16 <- t(res.si.sl16[1:25, ])
...
stopCluster(cl)
res.r1.sl.sim16 <- t(res.r1.sl16[1:26, ])
...
stopCluster(cl)
res.fs.sl.sim16 <- t(res.fs.sl16[1:25, ])
se.fs.sl.sim16 <- t(res.fs.sl16[26:44, ])
...
stopCluster(cl)
res.na.tl.sim16 <- t(res.na.tl16[1:13, ])
...
stopCluster(cl)
res.re.tl.sim16 <- t(res.re.tl16[1:25, ])
...
stopCluster(cl)
res.si.tl.sim16 <- t(res.si.tl16[1:25, ])
...
stopCluster(cl)
res.r1.tl.sim16 <- t(res.r1.tl16[1:26, ])
...
stopCluster(cl)
res.fs.tl.sim16 <- t(res.fs.tl16[1:25, ])
se.fs.tl.sim16 <- t(res.fs.tl16[26:44, ])

save(res.na.sl.sim16,se.na.sl.sim16,
     res.re.sl.sim16,se.re.sl.sim16,
     res.si.sl.sim16,se.si.sl.sim16,
     res.r1.sl.sim16,se.r1.sl.sim16,
     res.fs.sl.sim16,se.fs.sl.sim16,
     res.na.tl.sim16,se.na.tl.sim16,
     res.re.tl.sim16,se.re.tl.sim16,
     res.si.tl.sim16,se.si.tl.sim16,
     res.r1.tl.sim16,se.r1.tl.sim16,
     res.fs.tl.sim16,se.fs.tl.sim16,
     file = "res_scenario16.Rdata")

#### scenario 17: 10 covariates & tte outcome & total sample size = 1400 ####
...
stopCluster(cl)
res.na.sl.sim17 <- t(res.na.sl17[1:25, ])
...
stopCluster(cl)
res.re.sl.sim17 <- t(res.re.sl17[1:25, ])
...
stopCluster(cl)
res.si.sl.sim17 <- t(res.si.sl17[1:25, ])
...
stopCluster(cl)
res.r1.sl.sim17 <- t(res.r1.sl17[1:26, ])
...
stopCluster(cl)
res.fs.sl.sim17 <- t(res.fs.sl17[1:25, ])
se.fs.sl.sim17 <- t(res.fs.sl17[26:44, ])
...
stopCluster(cl)
res.na.tl.sim17 <- t(res.na.tl17[1:13, ])
...
stopCluster(cl)
res.re.tl.sim17 <- t(res.re.tl17[1:25, ])
...
stopCluster(cl)
res.si.tl.sim17 <- t(res.si.tl17[1:25, ])
...
stopCluster(cl)
res.r1.tl.sim17 <- t(res.r1.tl17[1:26, ])
...
stopCluster(cl)
res.fs.tl.sim17 <- t(res.fs.tl17[1:25, ])
se.fs.tl.sim17 <- t(res.fs.tl17[26:44, ])

save(res.na.sl.sim17,se.na.sl.sim17,
     res.re.sl.sim17,se.re.sl.sim17,
     res.si.sl.sim17,se.si.sl.sim17,
     res.r1.sl.sim17,se.r1.sl.sim17,
     res.fs.sl.sim17,se.fs.sl.sim17,
     res.na.tl.sim17,se.na.tl.sim17,
     res.re.tl.sim17,se.re.tl.sim17,
     res.si.tl.sim17,se.si.tl.sim17,
     res.r1.tl.sim17,se.r1.tl.sim17,
     res.fs.tl.sim17,se.fs.tl.sim17,
     file = "res_scenario17.Rdata")


#### scenario 18: 10 covariates & tte outcome & total sample size = 700 ####
...
stopCluster(cl)
res.na.sl.sim18 <- t(res.na.sl18[1:25, ])
...
stopCluster(cl)
res.re.sl.sim18 <- t(res.re.sl18[1:25, ])
...
stopCluster(cl)
res.si.sl.sim18 <- t(res.si.sl18[1:25, ])
...
stopCluster(cl)
res.r1.sl.sim18 <- t(res.r1.sl18[1:26, ])
...
stopCluster(cl)
res.fs.sl.sim18 <- t(res.fs.sl18[1:25, ])
se.fs.sl.sim18 <- t(res.fs.sl18[26:44, ])
...
stopCluster(cl)
res.na.tl.sim18 <- t(res.na.tl18[1:13, ])
...
stopCluster(cl)
res.re.tl.sim18 <- t(res.re.tl18[1:25, ])
...
stopCluster(cl)
res.si.tl.sim18 <- t(res.si.tl18[1:25, ])
...
stopCluster(cl)
res.r1.tl.sim18 <- t(res.r1.tl18[1:26, ])
...
stopCluster(cl)
res.fs.tl.sim18 <- t(res.fs.tl18[1:25, ])
se.fs.tl.sim18 <- t(res.fs.tl18[26:44, ])

save(res.na.sl.sim18,se.na.sl.sim18,
     res.re.sl.sim18,se.re.sl.sim18,
     res.si.sl.sim18,se.si.sl.sim18,
     res.r1.sl.sim18,se.r1.sl.sim18,
     res.fs.sl.sim18,se.fs.sl.sim18,
     res.na.tl.sim18,se.na.tl.sim18,
     res.re.tl.sim18,se.re.tl.sim18,
     res.si.tl.sim18,se.si.tl.sim18,
     res.r1.tl.sim18,se.r1.tl.sim18,
     res.fs.tl.sim18,se.fs.tl.sim18,
     file = "res_scenario18.Rdata")
